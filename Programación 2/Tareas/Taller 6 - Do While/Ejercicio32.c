...
int main()
{
    ...
    printf("Ingrese el valor de X en la formula cosH(x): ");
    scanf("%lf", &x);
    ...
    printf("cosH(%.1f) = %.6f\n", x, cosenoH(x, terminos));
    return 0;
}

// Funcíon de cosenoH(x)
double cosenoH(double x, int terminos)
{
    // Declaramos la variable que guardará la sumatoria de las series de taylor
    double resultado = 0;
    ...
    do
    {
        resultado += pow(x, (2 * n)) / tgamma(2 * n + 1);
