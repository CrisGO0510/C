...
int main()
{
    ...
    printf("Ingrese el valor de X que desea obtener en la función cosh(x): ");
    scanf("%i", &x);
    ...
    // Imprimimos lo que nos retorne la función de cosH
    printf("%f", cosH(x, 0, term));
...
// Función para obtener el valor de la función de cosh(x) usando los términos ingresados por el usuario en la serie de taylor
/*
...
*/
float cosH(int x, int den, int ter)
{
    if (den < ter)
    {
        // Si "den" es menor que "ter" significa que aún falta recorrido por llegar
        return (pow(x, den * 2) / fac(den * 2)) + cosH(x, den + 1, ter);
    }
    else
    {
