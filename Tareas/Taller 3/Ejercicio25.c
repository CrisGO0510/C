...
int main()
{
    ...
    printf("Ingrese el valor de X que desea obtener en la función sen(x): ");
    scanf("%i", &x);
    ...
    // Imprimimos lo que nos retorne la función de seno
    printf("%.9f", seno(x, 0, term));
...
// Función para obtener el valor de la función de sen(x) usando los términos ingresados por el usuario en la serie de taylor
/*
...
*/
float seno(int x, int den, int ter)
{
    if (den < ter)
    {
        // Si den es menor que ter significa que aún falta recorrido por llegar
        return ((pow(-1, den) * pow(x, den * 2 + 1)) / fac(den * 2 + 1)) + seno(x, den + 1, ter);
    }
    else
    {
